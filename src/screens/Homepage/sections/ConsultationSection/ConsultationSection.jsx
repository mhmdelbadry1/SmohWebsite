...
export const ConsultationSection = () => {
  ...
  const policyLinks = [
    { id: 'privacy', text: lang === 'ar' ? "سياسة الخصوصية" : "Privacy Policy", className: "[font-family:'Poppins',Helvetica]" },
    { text: "|", className: "[font-family:'SF_Pro_Text-Regular',Helvetica]" },
    { id: 'terms', text: lang === 'ar' ? "الشروط والأحكام" : "Terms and Conditions", className: "[font-family:'Poppins',Helvetica]" },
  ];
    ...
    <section className={`container flex flex-col items-center gap-8 md:gap-14 py-8 md:py-16 lg:py-20 w-full px-4 ${lang === 'ar' ? 'rtl' : 'ltr'}`}>
            ...
            {policyLinks.map((item, index) => (
              item.id ? (
                <button
                  ...
                  className={`${item.className} opacity-40 font-normal text-black text-xs lg:text-sm xl:text-base leading-[22px] whitespace-nowrap tracking-[0] text-center ${lang === 'ar' ? 'lg:text-left' : 'lg:text-right'} hover:opacity-70 hover:text-[#4C31AF] transition-all duration-300 cursor-pointer bg-transparent border-none p-0`}
                >
                  {item.text}
                </button>
              ) : (
                <span
                  key={index}
                  className={`${item.className} opacity-40 font-normal text-black text-xs lg:text-sm xl:text-base leading-[22px] whitespace-nowrap tracking-[0] text-center ${lang === 'ar' ? 'lg:text-left' : 'lg:text-right'}`}
                >
                  {item.text}
                </span>
              )
      ...
      {isOverlayOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={overlayType === 'privacy' ? (lang === 'ar' ? 'سياسة الخصوصية' : 'Privacy Policy') : (lang === 'ar' ? 'الشروط والأحكام' : 'Terms and Conditions')}
        >
          {/* Backdrop */}
          <div 
            className={`absolute inset-0 bg-black/60 ${
              showBackdrop && isAnimating ? 'backdrop-blur-sm' : 'backdrop-blur-none'
            }`}
            style={{
              transition: 'backdrop-filter 400ms ease-out, background-color 300ms ease-out',
              backgroundColor: showBackdrop && isAnimating ? 'rgba(0, 0, 0, 0.6)' : 'rgba(0, 0, 0, 0)'
            }}
          ...
          {/* Panel */}
          <div 
            className={`relative bg-white rounded-2xl shadow-2xl max-h-[85vh] w-[92vw] md:w-[80vw] lg:w-[900px] overflow-hidden border border-gray-100 transform ${lang === 'ar' ? 'text-right' : 'text-left'}`}
            style={{
              ...
              {overlayType === 'privacy' ? (
                ...
                <div className="space-y-4 text-sm leading-7">
                  {lang === 'ar' ? (
                    <div dir="rtl">
                      ...
                      <p>تحترم شركة سمو للدعاية والإعلان خصوصية زوار ومستخدمي موقعها الإلكتروني (sumouadvs.com) وتلتزم بحماية بياناتهم الشخصية وفقًا لأفضل الممارسات والمعايير المعمول بها.</p>

                      <p className="font-semibold">2. جمع المعلومات</p>
                      <ul className="list-disc pr-5 space-y-1">
                        <li>البيانات التي تقدمها طوعًا (مثل: الاسم، البريد الإلكتروني، رقم الهاتف).</li>
                        <li>البيانات التي يتم جمعها تلقائيًا (مثل: عنوان الـ IP، نوع الجهاز، المتصفح، الصفحات التي تمت زيارتها).</li>
                      </ul>
                      ...
                      <ul className="list-disc pr-5 space-y-1">
                        ...
                        <li>إرسال العروض الترويجية والمستجدات (عند الموافقة).</li>
                        <li>تحليل أداء الموقع وتحسين الخدمات.</li>
                      </ul>

                      <p className="font-semibold">4. حماية البيانات</p>
                      <p>نلتزم باستخدام تدابير أمنية تقنية وإدارية مناسبة لحماية بياناتك من الوصول غير المصرح به أو التعديل أو الفقدان.</p>

                      <p className="font-semibold">5. مشاركة المعلومات</p>
                      <ul className="list-disc pr-5 space-y-1">
                        <li>إذا كان ذلك مطلوبًا بموجب القانون.</li>
                        <li>إذا كان ذلك ضروريًا لتقديم الخدمة من خلال شركاء موثوقين.</li>
                      </ul>

                      <p className="font-semibold">6. ملفات تعريف الارتباط (Cookies)</p>
                      <p>يستخدم موقعنا ملفات تعريف الارتباط لتحسين تجربتك. يمكنك تعطيلها من إعدادات المتصفح، ولكن قد يؤثر ذلك على بعض وظائف الموقع.</p>
                      ...
                      <p>قد يتم تحديث سياسة الخصوصية من وقت لآخر، وسيتم نشر النسخة المحدثة على هذه الصفحة.</p>
                    </div>
                  ) : (
                    <div dir="ltr">
                      <p className="font-semibold">1. Introduction</p>
                      <p>Sumou Advertising & Marketing respects the privacy of visitors and users of its website (sumouadvs.com) and is committed to protecting their personal data in accordance with best practices and applicable standards.</p>

                      <p className="font-semibold">2. Information Collection</p>
                      <ul className="list-disc pl-5 space-y-1">
                        <li>Information you voluntarily provide (e.g., name, email address, phone number).</li>
                        <li>Information automatically collected (e.g., IP address, device type, browser type, and pages visited).</li>
                      </ul>
                      ...
                      <ul className="list-disc pl-5 space-y-1">
                        ...
                        <li>To analyze website performance and enhance services.</li>
                      </ul>

                      <p className="font-semibold">4. Data Protection</p>
                      <p>We are committed to using appropriate technical and administrative measures to protect your data from unauthorized access, alteration, or loss.</p>

                      <p className="font-semibold">5. Information Sharing</p>
                      <ul className="list-disc pl-5 space-y-1">
                        <li>If required by law.</li>
                        <li>When necessary to provide services through trusted partners.</li>
                      </ul>

                      <p className="font-semibold">6. Cookies</p>
                      <p>Our website uses cookies to enhance your browsing experience. You can disable them in your browser settings, but this may affect certain website functionalities.</p>
                      ...
                      <p>This Privacy Policy may be updated from time to time, and the latest version will be posted on this page.</p>
                    </div>
                  )}
                </div>
              ) : (
                ...
                <div className="space-y-4 text-sm leading-7">
                  {lang === 'ar' ? (
                    <div dir="rtl">
                      <p className="font-semibold">1. المقدمة</p>
                      <p>باستخدامك لموقعنا أو خدماتنا، فإنك توافق على الالتزام بالشروط والأحكام الموضحة أدناه.</p>

                      <p className="font-semibold">2. استخدام الموقع</p>
                      <ul className="list-disc pr-5 space-y-1">
                        <li>يحق لك استخدام الموقع للأغراض المشروعة فقط.</li>
                        <li>يمنع نسخ أو إعادة توزيع أي محتوى دون إذن كتابي مسبق من شركة سمو.</li>
                      </ul>

                      <p className="font-semibold">3. الملكية الفكرية</p>
                      <p>جميع الحقوق محفوظة لشركة سمو للدعاية والإعلان، بما في ذلك النصوص، التصاميم، الصور، والشعارات.</p>

                      <p className="font-semibold">4. المحتوى والخدمات</p>
                      <ul className="list-disc pr-5 space-y-1">
                        <li>قد نقوم بتعديل أو إيقاف أي خدمة أو محتوى في أي وقت دون إشعار مسبق.</li>
                        <li>لا نضمن خلو الموقع من الأخطاء أو انقطاع الخدمة.</li>
                      </ul>

                      <p className="font-semibold">5. حدود المسؤولية</p>
                      <p>لا تتحمل شركة سمو أي مسؤولية عن أي أضرار مباشرة أو غير مباشرة تنتج عن استخدام الموقع أو الخدمات.</p>

                      <p className="font-semibold">6. الروابط الخارجية</p>
                      <p>قد يحتوي الموقع على روابط لمواقع خارجية، ولسنا مسؤولين عن محتوى أو ممارسات الخصوصية لتلك المواقع.</p>

                      <p className="font-semibold">7. القانون المعمول به</p>
                      <p>تخضع هذه الشروط والأحكام وتفسر وفقًا لقوانين المملكة العربية السعودية، وأي نزاعات تخضع لاختصاص محاكمها.</p>
                    </div>
                  ) : (
                    <div dir="ltr">
                      <p className="font-semibold">1. Introduction</p>
                      <p>By using our website or services, you agree to be bound by the terms and conditions outlined below.</p>

                      <p className="font-semibold">2. Website Use</p>
                      <ul className="list-disc pl-5 space-y-1">
                        <li>You may only use the website for lawful purposes.</li>
                        <li>Copying or redistributing any content without prior written consent from Sumou is prohibited.</li>
                      </ul>

                      <p className="font-semibold">3. Intellectual Property</p>
                      <p>All rights are reserved to Sumou Advertising & Marketing, including texts, designs, images, and logos.</p>

                      <p className="font-semibold">4. Content and Services</p>
                      <ul className="list-disc pl-5 space-y-1">
                        <li>We may modify or discontinue any service or content at any time without prior notice.</li>
                        <li>We do not guarantee the website will be error-free or that service will be uninterrupted.</li>
                      </ul>

                      <p className="font-semibold">5. Limitation of Liability</p>
                      <p>Sumou shall not be held liable for any direct or indirect damages resulting from the use of the website or its services.</p>

                      <p className="font-semibold">6. External Links</p>
                      <p>The website may contain links to external websites, and we are not responsible for the content or privacy practices of those sites.</p>

                      <p className="font-semibold">7. Governing Law</p>
                      <p>These terms and conditions are governed by and interpreted in accordance with the laws of the Kingdom of Saudi Arabia, and any disputes shall be subject to the jurisdiction of its courts.</p>
                    </div>
                  )}
                </div>
              )}
